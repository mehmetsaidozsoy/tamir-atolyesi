...
class TamirAtolyesiGUI:
    ...
    def search_repairs(self):
        ...
        try:
            ...
            for tamir in tamirler:
                try:
                    ucret = float(tamir[5]) if tamir[5] and str(tamir[5]).strip() != '' else 0
    ...
    def show_customer_report(self):
        ...
        def rapor_olustur():
            try:
                ...
                veriler = self.db.musteri_tamir_raporu(
                    baslangic.get(),
                    bitis.get()
                )
    ...
    def show_repair_report(self):
        ...
        def rapor_olustur():
            try:
                ...
                # Rapor verilerini getir
                veriler = self.db.tamir_raporu(
                    baslangic.get(),
                    bitis.get(),
                    None if durum.get() == "Tümü" else durum.get()
                )
    ...
    def generate_report(self):
        """Seçili raporu oluşturur"""
        try:
            # Tarih kontrolü
            baslangic = self.start_date.get()
            bitis = self.end_date.get()
            
            if not baslangic or not bitis:
                messagebox.showwarning("Uyarı", "Lütfen tarih aralığı seçin")
                return
            ...
            if rapor_turu == "Tamir Raporu":
                self.generate_repair_report(baslangic, bitis)
            elif rapor_turu == "Müşteri Raporu":
                self.generate_customer_report(baslangic, bitis)
            elif rapor_turu == "Gelir Raporu":
                self.generate_income_report(baslangic, bitis)
            elif rapor_turu == "Maliyet Analizi":
                self.generate_cost_report(baslangic, bitis)
                
        except Exception as e:
            messagebox.showerror("Hata", str(e))

    def generate_repair_report(self, baslangic, bitis):
        """Tamir raporu oluşturur"""
        # Sütunları sadece gerekirse ayarla
        columns = ("tarih", "musteri", "cihaz", "durum", "ucret")
        if tuple(self.report_tree["columns"]) != columns:
            self.report_tree["columns"] = columns
            self.report_tree.heading("tarih", text="Tarih")
            self.report_tree.heading("musteri", text="Müşteri")
            self.report_tree.heading("cihaz", text="Cihaz")
            self.report_tree.heading("durum", text="Durum")
            self.report_tree.heading("ucret", text="Ücret")
            self.report_tree.column("tarih", width=100)
            self.report_tree.column("musteri", width=200)
            self.report_tree.column("cihaz", width=300)
            self.report_tree.column("durum", width=100)
            self.report_tree.column("ucret", width=100)
        # Satırları temizle
        for item in self.report_tree.get_children():
            self.report_tree.delete(item)
        # Verileri getir
        tamirler = self.db.tamir_raporu(baslangic, bitis)
        for tamir in tamirler:
            self.report_tree.insert("", "end", values=(
                ...
                tamir[3],  # Cihaz
                tamir[4],  # Durum
                f"{tamir[5]:.2f} TL"  # Ücret
            ))

    def generate_customer_report(self, baslangic, bitis):
        """Müşteri raporu oluşturur"""
        columns = ("musteri", "tamir_sayisi", "toplam_ucret", "son_tamir")
        if tuple(self.report_tree["columns"]) != columns:
            self.report_tree["columns"] = columns
            self.report_tree.heading("musteri", text="Müşteri")
            self.report_tree.heading("tamir_sayisi", text="Tamir Sayısı")
            self.report_tree.heading("toplam_ucret", text="Toplam Ücret")
            self.report_tree.heading("son_tamir", text="Son Tamir")
            self.report_tree.column("musteri", width=200)
            self.report_tree.column("tamir_sayisi", width=100)
            self.report_tree.column("toplam_ucret", width=100)
            self.report_tree.column("son_tamir", width=100)
        for item in self.report_tree.get_children():
            self.report_tree.delete(item)
        musteriler = self.db.musteri_bazli_tamir_raporu(baslangic, bitis)
        for musteri in musteriler:
            self.report_tree.insert("", "end", values=(
                f"{musteri[2]} {musteri[3]}",  # Müşteri
                musteri[8],  # Tamir Sayısı
                f"{musteri[11]:.2f} TL",  # Toplam Ücret
                musteri[7]  # Son Tamir
            ))

    def generate_income_report(self, baslangic, bitis):
        """Gelir raporu oluşturur"""
        columns = ("kategori", "miktar")
        if tuple(self.report_tree["columns"]) != columns:
            self.report_tree["columns"] = columns
            self.report_tree.heading("kategori", text="Kategori")
            self.report_tree.heading("miktar", text="Miktar")
            self.report_tree.column("kategori", width=200)
            self.report_tree.column("miktar", width=100)
        for item in self.report_tree.get_children():
            self.report_tree.delete(item)
    ...
    def generate_cost_report(self, baslangic, bitis):
        """Maliyet analiz raporu oluşturur"""
        columns = ("cihaz", "iscilik", "parca", "toplam", "kar_orani")
        if tuple(self.report_tree["columns"]) != columns:
            self.report_tree["columns"] = columns
            self.report_tree.heading("cihaz", text="Cihaz")
            self.report_tree.heading("iscilik", text="İşçilik")
            self.report_tree.heading("parca", text="Parça")
            self.report_tree.heading("toplam", text="Toplam")
            self.report_tree.heading("kar_orani", text="Kâr Oranı")
            self.report_tree.column("cihaz", width=300)
            self.report_tree.column("iscilik", width=100)
            self.report_tree.column("parca", width=100)
            self.report_tree.column("toplam", width=100)
            self.report_tree.column("kar_orani", width=100)
        for item in self.report_tree.get_children():
            self.report_tree.delete(item)
        ...
        for maliyet in maliyetler:
            self.report_tree.insert("", "end", values=(
                ...
                f"%{((maliyet[12] - (maliyet[10] + maliyet[11])) / maliyet[12] * 100):.2f}"  # Kâr Oranı
            ))

    def show_customer_repairs(self):
        """Seçili müşterinin tamir geçmişini gösterir"""
        selected = self.customers_tree.selection()
        if not selected:
            return
            
        musteri_id = self.customers_tree.item(selected[0])["values"][0]
        musteri = self.db.musteri_getir(musteri_id)
        if not musteri:
            return
            
        dialog = tk.Toplevel(self.parent)
        dialog.title(f"Tamir Geçmişi - {musteri[2]} {musteri[3]}")
        dialog.geometry("800x600")
        ...
        tree = ttk.Treeview(
            main_frame,
            columns=("tarih", "cihaz", "sorun", "durum", "ucret"),
            show="headings"
        )
        
        tree.heading("tarih", text="Tarih")
        tree.heading("cihaz", text="Cihaz")
        tree.heading("sorun", text="Sorun")
        tree.heading("durum", text="Durum")
        tree.heading("ucret", text="Ücret")
        
        tree.column("tarih", width=100)
        tree.column("cihaz", width=200)
        tree.column("sorun", width=200)
        tree.column("durum", width=100)
        tree.column("ucret", width=100)
        ...
        tamirler = self.db.tamir_gecmisi(musteri_id)
        for tamir in tamirler:
            tree.insert("", "end", values=(
                getattr(tamir, 'baslama_tarihi', ""),
                f"{getattr(tamir, 'cihaz', '')} - {getattr(tamir, 'marka', '')} {getattr(tamir, 'model', '')}",
                getattr(tamir, 'ariza', ""),
                getattr(tamir, 'durum', ""),
                f"{getattr(tamir, 'ucret', 0):.2f} TL"
            ))
            
        ttk.Button(dialog, text="Kapat", command=dialog.destroy).pack(pady=10)
    ...
    def setup_reports_tab(self):
        """Raporlar sekmesini hazırlar"""
        self.reports_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.reports_frame, text="Raporlar")
        
        # Rapor seçimi
        selection_frame = ttk.LabelFrame(self.reports_frame, text="Rapor Seçimi", padding="10")
        selection_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(selection_frame, text="Rapor Türü:").grid(row=0, column=0, padx=5, pady=5)
        self.report_type = ttk.Combobox(selection_frame, values=[
            "Tamir Raporu",
            "Müşteri Raporu",
            "Gelir Raporu",
            "Maliyet Analizi"
        ...
        ttk.Label(selection_frame, text="Başlangıç:").grid(row=0, column=2, padx=5, pady=5)
        self.start_date = ttk.Entry(selection_frame)
        self.start_date.grid(row=0, column=3, padx=5, pady=5)
        self.start_date.insert(0, datetime.now().strftime("%d.%m.%Y"))
        
        ttk.Label(selection_frame, text="Bitiş:").grid(row=0, column=4, padx=5, pady=5)
        self.end_date = ttk.Entry(selection_frame)
        ...
        # Rapor tablosu
        self.report_tree = ttk.Treeview(self.reports_frame)
        
        scrollbar = ttk.Scrollbar(self.reports_frame, orient="vertical", command=self.report_tree.yview)
        self.report_tree.configure(yscrollcommand=scrollbar.set)
        
        self.report_tree.pack(side="left", fill="both", expand=True, padx=10, pady=5)
    ...
    def load_repair_data(self, tamir):
        ...
        try:
            self.edit_mode = True
            self.tamir_id = tamir['id']
            
            self.customer_combo.set(f"{tamir['ad']} {tamir['soyad']}")
            self.device_type.set(tamir['cihaz_turu'])
            self.brand.insert(0, tamir['marka'])
            self.model.insert(0, tamir['model'])
            self.serial.insert(0, tamir['seri_no'])
            self.complaint.insert("1.0", tamir['sikayet'])
            self.problem.insert("1.0", tamir['ariza'])
            self.operations.insert("1.0", tamir['islemler'])
            
            self.labor_price.delete(0, "end")
            self.labor_price.insert(0, str(tamir['iscilik_ucreti']))
            self.parts_price.delete(0, "end")
            self.parts_price.insert(0, str(tamir['parca_ucreti']))
            
            self.total_price.configure(state="normal")
            self.total_price.delete(0, "end")
            self.total_price.insert(0, str(tamir['toplam_ucret']))
            self.total_price.configure(state="readonly")
            
            self.status.set(tamir['durum'])
